...
カテゴリー変数がエステティック属性にマップされると、ggplot2は自動的に、変数のユニークなレベルごと（この場合は3つの種族のぞれぞれ）に、ユニークな値（この場合はユニークな色）をエステティック属性に割り振ります。こ
の過程を**スケーリング**といいます。
...
変数の分布を可視化する方法は、変数のタイプがカテゴリーか数値かによって異なります。

### カテゴリー変数

変数が、値の小さな集合の中のどれらか一つだけを取れるとき、カテゴリー変数といいます。
カテゴリー変数の分布を検討するには、棒グラフが使えます。
棒の高さで`x`の各値ごとにいくつ観察されたかを表します。

```{r}
#| fig-alt: |
#|   ペンギンの種族ごとの頻度の棒グラフ。Adelie
#|   (約150)、Chinstrap (約90), Gentoo 
#|   (約125).
