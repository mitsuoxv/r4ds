...
ggplot2 は、プロットを記述、構築する首尾一貫したシステムである**グラフの文法**を実装しています。
ggplot2 を使えば、一つのシステムを学ぶとそれを多くの場所で適用できるので、多くのことをより速くできます。

本章では、**ggplot2** を使ってデータを可視化するやり方を学習します。
単純な散布図の作成から始めて、それを使ってggplot2 の根本的構築ブロックである、外観属性のマッピングと幾何オブジェクトを紹介します。
...
`ggplot()`関数の`mapping`引数で、データセットの変数をプロットの外観を決める属性 (**外観属性**) にどうマップするか指定します。
`mapping`引数は常に`aes()`関数で、その中で指定します。例
えば、`aes()`の引数`x`と`y`で、どの変数をx軸とy軸にマップするか指定します。
とりあえず、翼の長さを`x`外観属性に、体重を`y`外観属性にマップするだけにします。
...
そうするには、データを表すのにプロットが用いる幾何オブジェクトである**geom**を指定する必要があります。
...
### 外観属性とレイヤーを追加 {#sec-adding-aesthetics-layers}
...
そうするには、外観属性とgeomのどちらを修正する必要があるのでしょうか？
"`aes()`の中の外観属性のマッピング"を修正するのでは、と思ったとしたら、あなたはggplot2を使ったデータ可視化の手掛かりを得つつあるということです！
...
カテゴリ変数が外観属性にマップされると、ggplot2は自動的に、変数のユニークなレベルごと（この場合は3つの種族のぞれぞれ）に、ユニークな値（この場合はユニークな色）を外観属性に割り振ります。こ
の過程を**スケーリング**といいます。
...
外観属性を`ggplot()`で指定すると、*グローバル*レベルでの指定となり、重ねるレイヤーの全てに伝わります。
でも、ggplot2のgeom関数は皆、`mapping` 引数を取ることができ、そこで外観属性を指定すれば*local*レベルの指定として、グローバルレベルから引き継いだ指定に加えることができます。
...
なので、色に加えて、外観属性`shape`に`species`をマップします。
...
2.  以下の2つのプロットはどう違っていますか？
    `color`と`fill`、どちらの外観属性が、棒グラフの色を変えるのに適していますか？
...
関係を可視化するには、少なくとも2つの変数をプロットの外観属性にマップする必要があります。
...
さらに、`species`を`color`と`fill`の両方の外観属性にマップして、`alpha`外観属性を設定して密度曲線が覆う領域に透明度を加えています。
...
-   外観属性が変数の値に応じて変わるように可視化したいのなら、変数を外観属性に*マップ*する。
-   そうでなければ、外観属性の値を*設定*する。
...
これらの棒グラフを作成するには、棒を分ける変数を`x`外観属性に、棒の色を変える変数を`fill`外観属性にマップします。
...
@sec-adding-aesthetics-layers で見たように、さらなる変数を追加の外観属性にマップすることで、プロットに取り込むことができます。
...
しかし、プロットにあまりに多くの外観属性のマッピングを追加すると、ごたごたして、意味をくみ取るのが難しくなります。
カテゴリ変数には特に適していますが、別のやり方はプロットを、データのサブセットをそれぞれ表示するサブプロット、**ファセット**に分けることです。

プロットを1つの変数でファセットするには、`facet_wrap()`を使います。
`facet_wrap()`の第1引数は式[^data-visualize-3]で、変数名の前に`~` (チルダと読みます) を付けて作ります。
`facet_wrap()`に渡す変数はカテゴリ変数でなければなりません。

[^data-visualize-3]: ここでの"式"は`~`で作られるものの名前で、"方程式"の類義語ではありません。
...
2.  `mpg`データフレームを使って、`hwy`対`displ`の散布図を描きなさい。
    ...
    これらの外観属性は、カテゴリ変数がマップされた場合と数値変数がマップされた場合とで、どのように違った振る舞いをしますか？

3.  `hwy`対`displ`の散布図で、第3の変数を`linewidth`にマップしたら、どうなりますか？

4.  同じ変数を複数の外観属性にマップしたら、どうなりますか？
...
コンソールの左端をチェックします。`+`
となっていたら、Rはコマンドが未完と考え、それを完成させる次の入力を待っている、ということです。
この場合、ESCAPEを押して、現在のコマンドを放棄し、はじめからやり直すのが一番簡単です。
...
視化とは、データの変数を位置、色、サイズ、形などの外観属性にマップすることである、という考え方です。
次に、レイヤーを重ねて、プロットの複雑さを増し、プレゼンとして改良することを学びました。
また、1つの変数の分布を可視化したり、2つ以上の変数間の関係を可視化したりするのによく用いられるプロットを、外観属性のマッピングを追加したり、ファセットを使って複数の小さなプロットに分割したりして作成することを学びました。
