...
次に旋回して、`pivot_wider()`にピボット (笑) しましょう。`p`
...
調査対象のコア単位は組織 The core unit being studied is an organization, but each organization is spread across six rows, with one row for each measurement taken in the survey organization.
