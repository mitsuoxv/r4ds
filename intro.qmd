...
簡単に言えば、データが整理されていれば、各列は変数に、各行は観測になっています。
...
変換とは、（一つの都市の全員とか、昨年の全てのデータとか）興味のある観測に絞り込む、（距離と時間から速度を計算するなど）既存の変数から関数を使って新しい変数を作り出す、（カウントや平均など）一連の要約統計値を計算する、などのことです。
...
可視化はあなたを驚かせることができますが、人が見て解釈しないといけないので、観測がとても多いとあまり上手く行きません。

**モデル**は可視化を補完する道具です。
ひとたび十分に精密な質問を持ったなら、モデルを使って答えることができます。
モデルは根本的に数学的、あるいは、計算上の道具なので、観測が多くなっても一般的には上手く行きます。
