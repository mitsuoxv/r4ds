...
典型的なデータサイエンスのプロジェクトは、以下のように複数の段階から成るモデルで表せます。@
fig-ds-diagram
...
いうのも、80％の時間は繰り返し作業で退屈、残り20％の時間は訳のわからん作業で不満がたまるからです。
新しい主題を学びはじめる場所としてふさわしくないのです！
替わりに、読み込みと整頓が済んだデータの可視化と変換からはじめます。
こうすることで、いざ自分のデータを読み込んで整頓する際に、苦痛が努力に見合うものだと知っているので、動機を高く保てます。
