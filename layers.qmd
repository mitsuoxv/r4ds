...
本章では、その基礎を拡張して、レイヤー化されたグラフの文法を学習します。
...
もう一つ、棒グラフでは役に立たないけれど、散布図ではとても役に立つ調整のタイプがあります。
最初の散布図を思い出してみましょう。
データセットには234個の観測があるのに、プロットには126個の点しかないことに気付いていましたか？
...
元になった`hwy`の`displ`値は丸められているので、点はグリッド上に現れてお互いに重なっています。
この問題は**オーバプロット**として知られています。
このままでは、データの分布を見るのは困難です。
重なりはグラフ中で均等に散らばっているのか、それとも、一つの特殊な`hwy`と`displ`の組み合わせに109個の値が集中しているのか？

位置調整を"jitter"に設定すれば、このグリッドによる重複を回避することができます。
`position = "jitter"`は、各点に小さなランダム・ノイズを加えます。
2つの点が同じランダム・ノイズを受ける可能性は低いので、点は散らばることになります。
...
ランダムさを加えることでプロットを改良するのは奇妙な方法のように思えますが、グラフはごくわずかに不正確になる一方で、*大いに*分かりやすくなります。
これはとても役に立つやり方なので、`geom_point(position = "jitter")`を略記できる`geom_jitter()`をggplot2は用意しています。

位置調整についてさらに学習するには、`?position_dodge`、`?position_fill`、`?position_identity`、`?position_jitter`、`?position_stack`として、各調整と関連するヘルプ・ページを探してください。
...
3.  `geom_jitter()`のどの引数が揺らぎの程度を制御していますか？

4.  `geom_jitter()`と`geom_count()`を比較、対照しなさい。

5.  `geom_boxplot()`のデフォルトの位置調整は何か？
    `mpg`データセットを可視化して、実演しなさい。

## 座標システム

座標システムはおそらくggplot2で最も複雑な部分です。
デフォルトの座標システムはCartesian座標システム `coord_cartesian()` で、各点の場所を決めるのにx位置とy位置は独立しています。
他にも2つ、ときに役立つ座標システムがあります。

-   `coord_quickmap()`は、地理的地図向けに正しいアスペクト比を設定します。
    ggplot2で空間データをプロットする場合、とても重要です。
    本書には地図を議論するスペースがありませんが、*ggplot2: Elegant graphics for data analysis*の[Maps chapter](https://ggplot2-book.org/maps.html)でさらに学習できます。
    ...
    #| message: false
    #| fig-alt: |
    #|   ニュージーランドの境界線の2つの地図。アスペクト比が、
    #|   第1のプロットでは不正確で、第2のプロットでは正確。
...
-   `coord_polar()`は、極性の座標システムを使います。
    極性の座標システムは、棒グラフとCoxcombチャート間の興味深い関係を明らかにします。
    ...
    #| fig-asp: 1
    #| fig-alt: |
    #|   2つのプロット。左側はダイアモンドのclarityの棒グラフで、
    #|   右側は同じデータのCoxcombチャート。
...
1.  `coord_polar()`を使って、積み上げ棒グラフをパイチャートに変えなさい。

2.  `coord_quickmap()`と`coord_map()`の違いは何か？

3.  以下プロットは市内燃料効率と高速道路燃料効率間の関係について何を語っているか？
    `coord_fixed()`が重要な理由は何か？
    `geom_abline()`は何をしているか？
...
@sec-ggplot2-calls で学んだグラフのテンプレートに、位置調整 position、統計変換 stat、座標システム coordinateとファセット facetを加えることで、以下のように拡張できます。
...
新しいテンプレートは、上でブランケットで囲んだ7つのパラメータを取ります。
実際には、グラフを作るのに、7つ全てのパラメータを書かねばならないことはまれです。D
ATA、MAPPINGS、GEOM_FUNCTION以外のパラメータは、ggplot2が役立つデフォルトを提供してくれるからです。

テンプレートの7つのパラメータは、プロットを構築する正式なシステムである、グラフの文法を構成しています。
グラフの文法はある洞察に基づいています。*ど*
*んな*プロットも、データセット、geom、マッピングの集合、stat、位置調整、座標システム、ファセット、テーマの組み合わせでユニークに描くことができる、という洞察です。

これがどう機能するか見るには、基本的なプロットをゼロから構築する方法を検討してみてください。デ
ータセットからはじめて、(statを使って) 表示したい情報に変換します。
次に、変換したデータの各観測を表示する幾何オブジェクトを選びます。
それから、geomの外観属性を使って、データ中の変数を表します。
各変数の値を外観属性の水準へマップすることでしょう。
こららのステップは @fig-visualization-grammar に図解されています。
次に、xとy変数の値を表示するためにオブジェクトの場所 (それ自体、外観属性です) を使ってgeomを置く、座標システムを選びます。

```{r}
#| label: fig-visualization-grammar
#| echo: false
#| fig-alt: |
#|   生データから頻度表までの各ステップを実演する図。
#|   頻度表の各行は、一つのcut水準とそのcut水準のダイアモンド数を示す
#|   カウントを表す。次に、こららの値は棒の高さにマップされる。
#| fig-cap: |
#|   生データから頻度表を経て棒グラフになるまでを実演する図。
#|   棒の高さは頻度を表している。

knitr::include_graphics("images/visualization-grammar.png")
```

この段階でグラフは完成していますが、さらに座標システム内でgeomの位置を調整 (位置調整) したり、グラフをサブプロットに分けたり (ファセット) できます。
また、1つ以上のレイヤーを追加してプロットを拡張することもできます。各
レイヤーはデータセット、幾何オブジェクト、マッピング、統計変換と位置調整を使います。

この方法を使えば、想像できる*どんな*プロットも構築できます。
言い換えれば、本章で学んだコードのテンプレートを使えば、何十万ものユニークなプロットを構築できるということです。

ggplot2の理論的基礎についてさらに学習したければ、ggplot2の理論を詳細に記述した学術論文"[The Layered Grammar of Graphics](https://vita.had.co.nz/papers/layered-grammar.pdf)"を読むと楽しめるでしょう。

## まとめ

本章では、レイヤー化されたグラフの文法を学習しました。簡
単なプロットを構築する外観属性と幾何オブジェクトからはじめて、プロットをサブプロットに分けるファセット、geomが計算される方法を理解する統計変換、geomが重なるのを回避したいときなどに位置を細かく制御する位置調整、そして、`x`と`y`の意味を根本的に変えられる座標システムと学んできました。
まだ触れていないたった一つのレイヤーがテーマ theme ですが、@sec-themes で紹介します。

ggplot2機能の完全な概要が得られる2つのとても役立つ資源は、ggplot2 cheatsheet ( <https://posit.co/resources/cheatsheets>で見つけられます) と、ggplot2 package website ([https://ggplot2.tidyverse.org](https://ggplot2.tidyverse.org/))です。

本章からの大切な教訓は、ggplot2が提供するgeomでは足りないと感じたときは、誰か他の人が既にあなたの問題を解いていて、ggplot2の拡張パッケージを作成してgeomを提供してくれているか、調査することがいつもお薦めだということです。
