...
#| fig-align: "center"
#| fig-cap: |
#|   Rには組み込み形が25個あり、数値で特定されている。重複している
#|   ように見えるものがいくつかある。例えば、0、15、22はどれも四角。
#|   違いは外観属性`color`と`fill`の相互交流にある。中が空の形 (0--14) は
#|   境界線が`color`で決まり、中が黒で埋まっている形 (15--20) は中の色ごと
#|   `color`で決まり、中が赤で埋まっている形 (21--24) は境界線が`color`で
#|   中の色は`fill`で決まる。似た形が隣り合うよう形には番号が付けられている。
#| fig-alt: |
