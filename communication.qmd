...
他者にデータがどんなものか素早く分かってもらうためには、プロットを可能な限り自己説明的にすることに相当の努力を注ぎ込む必要があります。
本章では、そのためにggplot2が提供してくれている道具をいくつか学習します。

本章は、良いグラフを作成するのに必要な道具にフォーカスします。
あなたは何がしたいか分かっていて、ただそうする方法を知る必要があることを前提とします。
...
可視化作成の機構は教えていませんが、その替わり、効果的なグラフを作成するために考える必要があることにフォーカスしています。

### 準備するもの

本章では、再びggplot2にフォーカスします。
また、dplyrを少々使ったデータ操作、**尺度 (scale)**を使ったbreaks、labels、transformations、palettesのデフォルトを上書きも行いますし、**ggrepel** ([https://ggrepel.slowkow.com](https://ggrepel.slowkow.com/)) by Kamil Slowikowski と **patchwork** ([https://patchwork.data-imaginist.com](https://patchwork.data-imaginist.com/)) by Thomas Lin Pedersen など2、3のggplot2拡張パッケージも使います。
