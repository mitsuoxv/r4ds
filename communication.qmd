...
@sec-exploratory-data-analysis では、*探索*のための道具としてプロットを使う方法を学習しました。
探索のためにプロットを作成するときは、結果を見るまでもなく、プロットがどの変数を表示しているか知っていました。
目的を持ってプロットを作成し、ざっと見て、そして次のプロットへと移って行く。
多くの分析過程では、何十も何百もプロットを作成して、ほとんどは直ちに捨て去ります。

データを理解できたら、その理解を他者へ*伝達*する必要があります。
聴衆は、あなたが持つ背景情報をおそらく共有しておらず、あなたほど深くデータに注力もしていません。
他者にデータがどんなものか素早く分かってもらうためには、プロットを可能な限り自己説明的にすべきで、相当の努力を注ぎ込む必要があります。
本章では、そのためにggplot2が提供してくれる道具をいくつか学習します。

本章は、良いグラフを作成するのに必要な道具にフォーカスします。
あなたは何がしたいか分かっていて、そうする方法をただ知る必要があることを前提とします。
そのため、本章と一般的な可視化の優れた本を一緒に読むことを強くお薦めします。
[The Truthful Art](https://www.amazon.com/gp/product/0321934075/), by Albert Cairo は特に良い本です。
可視化作成の機構は教えてくれませんが、その替わり、効果的なグラフを作成するための要検討事項にフォーカスしています。

### 準備するもの

本章では、再びggplot2にフォーカスします。
また、dplyrを少々使ってデータ操作、**尺度 (scale)**を使ってbreaks、labels、transformations、palettesのデフォルト上書きも行いますし、**ggrepel** ([https://ggrepel.slowkow.com](https://ggrepel.slowkow.com/)) by Kamil Slowikowski と **patchwork** ([https://patchwork.data-imaginist.com](https://patchwork.data-imaginist.com/)) by Thomas Lin Pedersen などのggplot2拡張パッケージも使います。
まだこれらのパッケージをインストールしていないなら、`install.packages()`を使って忘れずにインストールしておいてください。
...
探索のためのグラフを解説のためのグラフに転換するのに、最もはじめやすいのは良いラベルを付けることです。
`labs()`関数を使って、ラベルを付けられます。

```{r}
#| message: false
#| fig-alt: |
#|   車の高速道路での燃料効率、対、排気量の散布図。
#|   点は車のクラスごとに違った色になっている。
#|   車の高速道路での燃料効率と排気量間の関係の軌道に沿った滑らかな曲線が
#|   重ねられている。
#|   x軸には"Engine displacement (L)"と、y軸には"Highway fuel economy (mpg)"と
#|   ラベルが付けられている。凡例のラベルは"Car type"。プロットのタイトルは
#|   "Fuel efficiency generally decreases with engine size"。
#|   サブタイトルは"Two seaters (sports cars) are an exception because of their light weight"、
#|   キャプションは"Data from fueleconomy.gov"。
...
プロット・タイトルの目的は、主要な発見を要約することです。
プロットを描写するだけのタイトル、例えば、"A scatterplot of engine displacement vs. fuel economy"、は避けましょう。

もっとテキストを加える必要があるなら、他に2つ役に立つラベルがあります。`s`
`ubtitle`はタイトルの下に小さめのフォントで追加の詳細を加え、`caption`はプロットの右下にテキストを加え、データの出所を記述するのにしばしば使われます。
`labs()`を使って、軸や凡例のタイトルを変更することもできます。
短い変数名に替えて、より詳細な記述にして、単位を含めることをお薦めします。

文字列の替わりに、算式も使えます。`"`
`"`を`quote()`に変えるだけです。利
用可能なオプションについて読むには、`?plotmath`としてください。
...
#| fig-width: 3
#| fig-alt: |
#|   x軸とy軸のラベルが算式になっている散布図。
#|   x軸のラベルはx_i、
#|   y軸のラベルはsum of x_i  squared, for i from 1 to nの意味。
...
1.  燃料効率データのプロットを一つ作って、`title`、`subtitle`、`caption`、`x`、`y`、`color`に独自のラベルを付けなさい。

2.  燃料効率データを使って、以下プロットを再現しなさい。
    点の色と形、両方が駆動方式ごとに違っていることに注意。
...
3.  先月作成した探索のためのグラフを取り出して、他の人が理解しやすいよう、分かりやすいタイトルを付けなさい。

## 注釈

プロットの主要な構成要素にラベルを付けることに加えて、個々の観測や観測のグループにラベルを付けることはしばしば役に立ちます。
手元にある最初の道具は、`geom_text()`です。
`geom_text()`は`geom_point()`に似ていますが、もう一つの外観属性`label`を有しています。
これでプロットにテキストのラベルを追加することができます。

ラベルの出どころになりえる所が2カ所あります。
第1に、ラベルを提供してくれているtibbleかもしれません。
以下プロットのために、駆動方式ごとに最大排気量の車を引き出して、その情報を`label_info`と名付けた新しいデータフレームに保存します。
...
次に、この新しいデータフレームを使って、3つのグループに直接ラベルを付けて、凡例をプロットに直接置いたラベルに交換します。
`fontface`と`size`の引数を使って、テキスト・ラベルの見栄を好きなように調整できます。
`size`はプロットの他のテキストよりも大きく、`fontface`はボールドにしています。
(`theme(legend.position = "none"`) で、全ての凡例表示を消しています。こ
れについては、少し先でお話しします。

```{r}
#| fig-alt: |
#|   高速道路での燃料効率、対、排気量の散布図。
#|   点は駆動方式ごとに違った色になっている。
#|   駆動方式ごとに滑らかな曲線が重ねられている。
#|   テキスト・ラベルが曲線を、front-wheel、rear-wheel、4-wheelと
#|   特定している。
...
ラベルの配置を制御するために、`hjust` (horizontal justification) と`vjust` (vertical justification) を使っています。

しかし、上で作成した注釈付きプロットは、ラベル同士が重なり、点とも重なっており、読みづらいです。
ggrepelパッケージの`geom_label_repel()`関数を使えば、これらの問題の両方を解決できます。
この役に立つパッケージは自動的にラベルを調整して、重ならないようにしてくれます。
...
#|   駆動方式ごとに滑らかな曲線が重ねられている。
#|   テキスト・ラベルが曲線を、front-wheel、rear-wheel、4-wheelと
#|   特定している。ラベルは白地のボックスで、重ならない位置にある。
...
同じアイディアを用いて、ggrepelパッケージの`geom_text_repel()`を使えば、プロット上の特定の点を強調することもできます。
もう一つ手軽なテクニックがここでは使われています。
ラベルを付けた点をさらに強調するために、レイヤーを重ねて大きな空洞のある点を追加しています。

```{r}
#| fig-alt: |
#|   高速道路での燃料効率、対、排気量の散布図。
#|   高速道路での燃料効率が40超と、排気量が5超で高速道路での燃料効率が20超の
#|   点は、赤い空洞のある丸の付いた赤丸で、車のモデル名のラベルが付いている。
...
プロットに注釈を付ける助けとなるgeomは、`geom_text()`や`geom_label()`だけでなく、ggplot2には他にも多くあることを覚えておきましょう。
2、3のアイディアは、

-   `geom_hline()`と`geom_vline()`を使って、参照線を加える。
    線は厚く (`linewidth = 2`)、白色 (`color = white`) にすることが多く、主なデータ・レイヤーの下に描く。
    データから注目をそらすことなく、見やすくすることができる。

-   `geom_rect()`を使って、興味のある点を長方形で囲う。
    長方形の境界線は外観属性`xmin`、`xmax`、`ymin`、`ymax`で指定する。
    替わりの方法は、[ggforceパッケージ](https://ggforce.data-imaginist.com/index.html) で探す。特
    に[`geom_mark_hull()`](https://ggforce.data-imaginist.com/reference/geom_mark_hull.html)を使えば、点のサブセットを外皮で囲んで注釈とできる。

-   `geom_segment()`で`arrow`引数を使って、点に矢印を付けて注目をひく。
    外観属性`x`と`y`で始点を、`xend`と`yend`で終点を指定する。

プロットに注釈を加えるもう一つ手軽な関数は`annotate()`です。
経験則として、一般的にデータのサブセットを強調するにはgeomが役立ち、プロットに1つ、あるいは、2、3の注釈要素を加えるには`annotate()`が役立ちます。

`annotate()`を使って実演するため、プロットに加えるテキストをいくつか作りましょう。
テキストが少々長いので、`stringr::str_wrap()`に1行当たりの文字数を与えて、自動的に改行させています。
...
次に、注釈レイヤーを2つ追加します。一
つはlabel geomを、もう一つはsegment geomを使います。
どちらのgeomでも外観属性`x`と`y`が注釈の始点を特定し、segment geomではさらに`xend`と`yend`が終点を特定しています。
また、segmentは矢印のスタイルにしています。

```{r}
#| fig-alt: |
#|   車の高速道路での燃料効率、対、排気量の散布図。
#|   右下がりの赤い矢印が点のトレンドに沿っており、
#|   "Larger engine sizes tend to have lower fuel economy"という注釈文が
#|   矢印の隣に置かれている。
#|   矢印と注釈文は赤色。
...
注釈は、あなたの可視化から主に覚えてもらいたいことや、興味深い特徴を伝達するための強力な道具です。
唯一の限界はあなたの想像力 (と、見た目が楽しいように注釈を位置付ける忍耐力) です！

### 練習問題

1.  `geom_text()`で位置を無限にして、テキストをプロットの四隅に置きなさい。

2.  `annotate()`を使って、tibbleを作ることなく、最後のプロットの真ん中にpoint geomを追加しなさい。
    点の形、サイズ、色を好きなように調整しなさい。

3.  `geom_text()`でのlabelsはファセットすると、どうなる？
    一つのファセットだけにラベルを加えるには、どうする？
    ファセットごとに違うラベルを置くには、どうする？
    (ヒント：`geom_text()`に渡されるデータセットについて考えよ。)

4.  `geom_label()`で、背景の箱の外見を制御している引数は何？

5.  `arrow()`の4つの引数は何？
    その機能は？
    最も重要なオプションを実演する一連のプロットを作成しなさい。
